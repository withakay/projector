...
The `projector research` command group provides domain research capabilities that help teams investigate technology choices, feature landscapes, architecture decisions, and potential pitfalls before proposing changes.

## Requirements

### Requirement: Research initialization

The CLI SHALL initialize the `.projector/research/` directory structure with templates for structured domain investigation.

#### Scenario: Initialize research directory

- **WHEN** executing `projector research init`
- **THEN** create the `.projector/research/` directory if it does not exist
- **AND** create the `.projector/research/investigations/` subdirectory
...
- **WHEN** executing `projector research status`
- **THEN** check for existence of `.projector/research/investigations/*.md` files
...
- **AND** print a hint to run `projector research init` if the directory structure is missing
...
- **AND** specify that findings should be written to `.projector/research/investigations/stack-analysis.md`
...
- **AND** specify that findings should be written to `.projector/research/investigations/feature-landscape.md`
...
- **AND** specify that findings should be written to `.projector/research/investigations/architecture.md`
...
- **AND** specify that findings should be written to `.projector/research/investigations/pitfalls.md`
...
- **WHEN** generating research command templates
- **THEN** create a template file with instructions for:
  - Reading all investigation files in `.projector/research/investigations/`
...
- **AND** specify that the summary should be written to `.projector/research/SUMMARY.md`
...
The CLI SHALL integrate research capabilities with the broader Projector workflow, enabling research to precede proposal creation.
...
- **THEN** suggest running `projector research init` to create research structure
...
- **AND** recommend referencing `.projector/research/SUMMARY.md` in the proposal for context
...
- **WHEN** the `.projector/research/` directory cannot be created due to permissions or filesystem errors
...
- **WHEN** executing `projector research status` and investigation files are missing
- **THEN** display a warning that investigations are incomplete
- **AND** suggest running `projector research init` to create templates
- **AND** list which investigation files are missing

### Requirement: Template quality

The CLI SHALL generate high-quality templates that provide clear guidance for structured research and follow Projector conventions.
