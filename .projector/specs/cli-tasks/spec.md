...
The `projector tasks` command group provides enhanced task management capabilities for change execution, including waves, verification criteria, completion tracking, and status management optimized for long-running AI-assisted development.
...
- **WHEN** executing `projector tasks init <change-id>`
- **THEN** create `.projector/changes/<change-id>/tasks.md` if it does not exist
...
- **WHEN** executing `projector tasks status <change-id>`
- **THEN** parse `.projector/changes/<change-id>/tasks.md`
...
- **WHEN** executing `projector tasks start <change-id> <task-id>`
- **THEN** read `.projector/changes/<change-id>/tasks.md`
...
- **WHEN** executing `projector tasks complete <change-id> <task-id>`
- **THEN** read `.projector/changes/<change-id>/tasks.md`
- **AND** find the task with the specified ID
...
- **WHEN** executing `projector tasks next <change-id>`
- **THEN** read `.projector/changes/<change-id>/tasks.md`
...
- **WHEN** executing `projector tasks next <change-id>` or `projector tasks start <change-id> <task-id>`
...
- **WHEN** `.projector/changes/<change-id>/tasks.md` cannot be read due to permissions or missing file
- **THEN** display an error message explaining the failure
- **AND** suggest checking file permissions or running `projector tasks init <change-id>` if the file is missing
...
- **AND** suggest running `projector tasks init <change-id>` to recreate with proper format
- **AND** provide specific details about what was malformed

#### Scenario: Task not found

- **WHEN** executing `projector tasks start <change-id> <task-id>` or complete with an invalid task ID
- **THEN** display an error message that the task ID was not found
- **AND** list available task IDs in the change
- **AND** suggest running `projector tasks status <change-id>` to see all tasks
