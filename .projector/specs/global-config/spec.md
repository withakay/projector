...
- **THEN** `getGlobalConfigDir()` returns `/custom/config/projector`
...
- **THEN** `getGlobalConfigDir()` returns `~/.config/projector` (expanded to absolute path)

#### Scenario: Windows platform
- **WHEN** the platform is Windows
- **AND** `%APPDATA%` is set to `C:\Users\User\AppData\Roaming`
- **THEN** `getGlobalConfigDir()` returns `C:\Users\User\AppData\Roaming\projector`
