...
The `projector init` command SHALL create a complete Projector directory structure in any project, enabling immediate adoption of Projector conventions with support for the officially supported AI assistants.
...
- **THEN** create the following directory structure under the configured Projector directory (default `.projector/`):
```
.projector/
├── project.md
├── AGENTS.md
...
- **THEN** generate `<projectorDir>/AGENTS.md` containing complete Projector instructions for AI assistants
- **AND** generate `<projectorDir>/project.md` with project context template

### Requirement: Projector Skills Installation

Projector agent skills are a core part of Projector and SHALL be installed automatically.

#### Scenario: Installing Projector skills
- **WHEN** `projector init` runs
- **THEN** install core Projector skills into `.claude/skills/<skill>/SKILL.md` within the repo
- **AND** apply the configured Projector directory name inside each skill template
- **AND** include at least `projector-proposal`, `projector-apply`, `projector-archive`, `projector-research`, and `projector-review`

### Requirement: AI Tool Configuration

The command SHALL configure supported AI coding assistants with Projector instructions using a grouped selection experience.
...
- **THEN** create or update `CLAUDE.md` in the project root directory (not inside the Projector directory)
- **AND** populate the managed block with a short stub that points teammates to `@/<projectorDir>/AGENTS.md`

#### Scenario: Configuring OpenCode
- **WHEN** OpenCode is selected
- **THEN** create `.opencode/command/projector-proposal.md`, `.opencode/command/projector-apply.md`, `.opencode/command/projector-archive.md`, `.opencode/command/projector-research.md`, and `.opencode/command/projector-review.md`
- **AND** populate each file from shared templates so command text matches other tools

#### Scenario: Configuring Codex
- **WHEN** Codex is selected
- **THEN** create global prompt files at `~/.codex/prompts/projector-proposal.md`, `~/.codex/prompts/projector-apply.md`, `~/.codex/prompts/projector-archive.md`, `~/.codex/prompts/projector-research.md`, and `~/.codex/prompts/projector-review.md` (or under `$CODEX_HOME/prompts` if set)
...
- **THEN** create `.github/prompts/projector-proposal.prompt.md`, `.github/prompts/projector-apply.prompt.md`, `.github/prompts/projector-archive.prompt.md`, `.github/prompts/projector-research.prompt.md`, and `.github/prompts/projector-review.prompt.md`
...
- **THEN** create new file with stub instructions wrapped in markers so the full workflow stays in `<projectorDir>/AGENTS.md`:
```markdown
<!-- PROJECTOR:START -->
...
- Full guidance lives in '@/<projectorDir>/AGENTS.md'.
...
- **WHEN** the configured Projector directory already exists
- **THEN** inform the user that Projector is already initialized, skip recreating the base structure, and enter an extend mode
...
- **THEN** include prompt: "Please explain the Projector workflow from <projectorDir>/AGENTS.md and how I should work with you on this project"
...
- **AND** instruct users to restart their coding assistant to ensure /projector commands appear
...
`projector init` SHALL allow users to add configuration files for new AI coding assistants after the initial setup.

#### Scenario: Configuring an extra tool after initial setup
- **GIVEN** a Projector directory already exists and at least one AI tool file is present
- **WHEN** the user runs `projector init` and selects a different supported AI tool
- **THEN** generate that tool's configuration files with Projector markers the same way as during first-time initialization
...
`projector init` SHALL summarize tool actions when initialization or extend mode completes.
...
`projector init` SHALL treat extend mode without new native tool selections as a successful refresh.

#### Scenario: Allowing empty extend runs
- **WHEN** Projector is already initialized and the user selects no additional natively supported tools
...
- **AND** only create the Projector directory structure and template files
...
- **WHEN** displaying CLI help for `projector init`
...
`projector init` SHALL always scaffold the root-level `AGENTS.md` hand-off so every teammate finds the primary Projector instructions.
