...
The `projector plan` command group provides project-level planning functionality, enabling teams to manage PROJECT.md, ROADMAP.md, and STATE.md files that persist across sessions and provide context for long-running AI-assisted development.

## Requirements

### Requirement: Project planning initialization

The CLI SHALL initialize the `.projector/planning/` directory structure with template files for project vision, roadmapping, and state tracking.

#### Scenario: Initialize planning directory

- **WHEN** executing `projector plan init`
- **THEN** create the `.projector/planning/` directory if it does not exist
...
- **WHEN** executing `projector plan status`
- **THEN** check for existence of `.projector/planning/PROJECT.md`, `.projector/planning/ROADMAP.md`, and `.projector/planning/STATE.md`
...
- **AND** print a hint to run `projector plan init` if any files are missing
...
- **WHEN** executing `projector state` or `projector state show`
- **THEN** read `.projector/planning/STATE.md` if it exists
- **AND** display the contents in a human-readable format
- **AND** print an error message and suggest running `projector plan init` if STATE.md does not exist

#### Scenario: Record a decision

- **WHEN** executing `projector state decision "[decision text]"`
...
- **WHEN** executing `projector state blocker "[blocker text]"`
...
- **WHEN** executing `projector state note "[note text]"`
...
- **WHEN** executing `projector plan milestone [milestone-name]`
- **THEN** append a new milestone section to `.projector/planning/ROADMAP.md` with the provided name
...
- **WHEN** executing `projector plan phase [milestone-name] [phase-name]`
...
- **WHEN** the `.projector/planning/` directory cannot be created due to permissions or filesystem errors
...
- **AND** suggest running `projector plan init` to recreate the template

### Requirement: Template quality

The CLI SHALL generate high-quality templates that follow Projector conventions and provide clear guidance for users.
