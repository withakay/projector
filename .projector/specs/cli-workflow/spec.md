...
The `projector workflow` command group provides YAML-based workflow orchestration capabilities, enabling teams to define, execute, and track complex multi-step workflows with waves, tasks, and checkpoints that work across AI coding assistants.

## Requirements

### Requirement: Workflow initialization

The CLI SHALL initialize the `.projector/workflows/` directory structure with example workflow definitions.

#### Scenario: Initialize workflows directory

- **WHEN** executing `projector workflow init`
- **THEN** create the `.projector/workflows/` directory if it does not exist
- **AND** create the `.projector/workflows/.state/` subdirectory for tracking execution state
...
- **AND** print a hint to run `projector workflow list` to see available workflows
...
- **WHEN** executing `projector workflow list`
- **THEN** scan `.projector/workflows/` for YAML files
...
- **AND** check `.projector/workflows/.state/` for execution status
- **AND** display execution status for each workflow (not started, in-progress, completed)
- **AND** print a hint to run `projector workflow show <name>` for details
...
- **WHEN** executing `projector workflow show <workflow-name>`
- **THEN** parse `.projector/workflows/<workflow-name>.yaml`
...
- **WHEN** executing `projector workflow run <workflow-name> --tool <tool-name> -v <key>=<value>`
- **THEN** parse `.projector/workflows/<workflow-name>.yaml`
...
- **AND** create or update `.projector/workflows/.state/<workflow-name>.json` with execution start time, current wave, and task status
...
- **WHEN** executing `projector workflow run <workflow-name>` and a state file exists
- **THEN** read `.projector/workflows/.state/<workflow-name>.json`
...
- **WHEN** executing `projector workflow status <workflow-name>`
- **THEN** read `.projector/workflows/.state/<workflow-name>.json` if it exists
...
- **THEN** update `.projector/workflows/.state/<workflow-name>.json` with:
...
- **WHEN** `.projector/workflows/<workflow-name>.yaml` cannot be read due to permissions or missing file
- **THEN** display an error message explaining the failure
- **THEN** suggest checking file permissions or running `projector workflow list` to see available workflows
- **AND** exit with code 1

#### Scenario: State file cannot be written

- **WHEN** `.projector/workflows/.state/<workflow-name>.json` cannot be written due to permissions or filesystem errors
- **THEN** display an error message explaining the failure
...
- **WHEN** executing `projector workflow run` with an unsupported tool
...
- **WHEN** executing `projector workflow run` without providing a required variable
