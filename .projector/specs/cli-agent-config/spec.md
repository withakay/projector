...
The `projector agent-config` command group provides agent configuration management capabilities, enabling teams to configure per-tool model selection, context budgets, and agent preferences for AI coding assistants.

## Requirements

### Requirement: Agent configuration initialization

The CLI SHALL initialize the `.projector/config.yaml` file with default configuration for AI tools.

#### Scenario: Initialize agent configuration

- **WHEN** executing `projector agent-config init`
- **THEN** create `.projector/config.yaml` if it does not exist
...
- **AND** print a hint to run `projector agent-config summary` to view current configuration
...
- **WHEN** executing `projector agent-config summary`
- **THEN** read `.projector/config.yaml` if it exists
...
- **AND** suggest running `projector agent-config init` to create configuration
...
- **AND** suggest using `projector agent-config summary` to see available paths

#### Scenario: Get nested configuration value

- **WHEN** executing `projector agent-config get <path>` with nested keys
...
- **WHEN** executing `projector agent-config set <path> <value>`
...
- **AND** write the updated configuration back to `.projector/config.yaml` atomically
...
- **WHEN** executing `projector agent-config set <path> <value>` with nested keys
...
- **WHEN** executing `projector agent-config set <path> true|false`
...
- **WHEN** executing `projector agent-config set <path> <number>`
...
- **WHEN** generating `.projector/config.yaml`
...
- **WHEN** `.projector/config.yaml` cannot be read due to permissions or malformed YAML
- **THEN** display an error message explaining the failure
- **AND** suggest checking file permissions or running `projector agent-config init` to recreate
- **AND** exit with code 1

#### Scenario: Config file cannot be written

- **WHEN** `.projector/config.yaml` cannot be written due to permissions or filesystem errors
- **THEN** display an error message explaining the failure
...
- **WHEN** executing `projector agent-config get <path>` or `set <path> <value>` with an invalid path
...
- **WHEN** reading `.projector/config.yaml` and it contains YAML syntax errors
- **THEN** display an error message indicating the specific syntax error
- **AND** suggest correcting the YAML syntax or running `projector agent-config init` to recreate
- **AND** exit with code 1

### Requirement: Configuration integration

The CLI SHALL integrate agent configuration with other Projector commands and workflows.

#### Scenario: Use configuration in other commands

- **WHEN** executing other Projector commands that interact with AI tools
- **THEN** read `.projector/config.yaml` to retrieve tool-specific settings
...
- **WHEN** executing `projector workflow run` with a specified tool
- **THEN** read tool-specific configuration from `.projector/config.yaml`
