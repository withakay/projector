...
- **THEN** show error "No deltas found" with guidance:
  ...
  - Remind authors that files must live under `.projector/changes/{id}/specs/<capability>/spec.md`
  - Include an explicit note: "Spec delta files cannot start with titles before the operation headers"
  - Suggest running `projector change show {id} --json --deltas-only` for debugging
...
- **THEN** include expected header names and a minimal skeleton:
  ...
  - Mention the quick-reference section in `.projector/AGENTS.md` as the authoritative template
...
- **THEN** emit an error explaining that `### Requirement:` lines must be followed by narrative text before any `#### Scenario:` headers
  ...
  - Reference the pre-validation checklist in `.projector/AGENTS.md`
...
- Source file path (`projector/changes/{id}/proposal.md`, `.../specs/{cap}/spec.md`)
...
- **AND** use `.projector/` for all file path references
...
- **THEN** print "Next steps" with 2-3 targeted bullets and suggest `projector change show <id> --json --deltas-only`
...
- **WHEN** executing `projector validate` without arguments
...
- **GIVEN** stdin is not a TTY or `--no-interactive` is provided or environment variable `PROJECTOR_INTERACTIVE=0`
- **WHEN** executing `projector validate` without arguments
...
- **WHEN** executing `projector validate <item-name>`
...
- **WHEN** executing `projector validate --all`
- **THEN** validate all changes in `.projector/changes/` (excluding archive)
- **AND** validate all specs in `.projector/specs/`
...
- **THEN** include all change proposals under `.projector/changes/`
- **AND** exclude the `.projector/changes/archive/` directory

- **WHEN** validating with `--specs`
- **THEN** include all specs that have a `spec.md` under `.projector/specs/<id>/spec.md`

#### Scenario: Validate all changes

- **WHEN** executing `projector validate --changes`
- **THEN** validate all changes in `.projector/changes/` (excluding archive)
...
- **WHEN** executing `projector validate --specs`
- **THEN** validate all specs in `.projector/specs/`
...
- **WHEN** executing `projector validate --all --strict`
...
- **WHEN** executing `projector validate --all --json`
...
- **WHEN** executing `projector validate --all --json` (or `--changes` / `--specs`)
...
- **AND** suggest passing `--type change` or `--type spec`, or using `projector change validate` / `projector spec validate`
...
- **WHEN** executing `projector validate --type change <item>`
- **THEN** treat `<item>` as a change ID and validate it (skipping auto-detection)

- **WHEN** executing `projector validate --type spec <item>`
...
- The CLI SHALL respect `PROJECTOR_INTERACTIVE=0` to disable prompts globally.
...
- **WHEN** `projector validate` is executed with `--no-interactive` or with environment `PROJECTOR_INTERACTIVE=0`
...
- **WHEN** running `projector validate <change-id>`
