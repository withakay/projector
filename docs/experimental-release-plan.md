# Spool Experimental Release Plan

This document outlines the plan to release the experimental artifact workflow system for user testing.

## Overview

The goal is to allow users to test the new artifact-driven workflow system alongside the existing Spool commands. This experimental system (`opsx`) provides a more granular, step-by-step approach to creating change artifacts.

## Three Workflow Modes

### 1. Old Workflow (Current Production)
- **Commands**: `/spool:proposal`, `/spool:apply`, `/spool:archive`
...
- **Commands**: Refactored `/spool:proposal` using schemas
...
**CLI Commands:**
The underlying CLI commands (`spool status`, `spool instructions`, etc.) remain unchanged. Only the slash command names change.
...
**Current State:**
- `.claude/commands/wf/start.md` - References non-existent `spool wf` commands
- `.claude/commands/wf/continue.md` - References non-existent `spool wf` commands
...
2. Generate two skills using the Agent Skills specification:
   - `spool-new-change/SKILL.md` - Start a new change with artifact workflow
   - `spool-continue-change/SKILL.md` - Continue working on a change (create next artifact)
3. Skills are added **alongside** existing `/spool:*` commands (not replacing)
...
**Tasks:**
- [x] Create skill template content for `spool-new-change` (based on current opsx:new)
- [x] Create skill template content for `spool-continue-change` (based on current opsx:continue)
...
**Note:** The `artifact-experimental-setup` command is temporary and will be merged into `spool init` once the experimental workflow is promoted to stable.
...
#
#   ✓ .claude/skills/spool-new-change/SKILL.md
#   ✓ .claude/skills/spool-continue-change/SKILL.md
#   ✓ .claude/skills/spool-apply-change/SKILL.md
#
# 📖 Usage:
#
...
#   • "I want to start a new Spool change to add <feature>"
#   • "Continue working on this change"
#
#   Claude will automatically use the appropriate skill.
#
# 💡 This is an experimental feature.
#    Feedback welcome at: https://github.com/withakay/Spool/issues
```
...
- [x] Verify CLI commands still work (they use `spool`, not `awf`)
...
**Test Scenario:**
Use a real Spool feature as the test case (dog-fooding).

**Test Flow:**
1. Run `spool artifact-experimental-setup` to create skills
2. Verify `.claude/skills/spool-new-change/SKILL.md` created
3. Verify `.claude/skills/spool-continue-change/SKILL.md` created
4. Verify `.claude/skills/spool-apply-change/SKILL.md` created
5. Ask Claude: "I want to start a new Spool change to add feature X"
6. Verify Claude invokes the `spool-new-change` skill
7. Verify change directory created at `spool/changes/add-feature-x/`
8. Verify proposal template shown
9. Ask Claude: "Continue working on this change"
10. Verify Claude invokes the `spool-continue-change` skill
...
20. Run `/spool:archive` to archive the change

**Validation Checklist:**
- [ ] `spool artifact-experimental-setup` creates correct directory structure
...
1. **What is the experimental workflow?**
   - A new way to create Spool changes step-by-step using Agent Skills
...
2. **How to set up experimental workflow**
   ...
   Note: This is a temporary command that will be integrated into `spool init` once promoted to stable.

3. **Available skills**
   - `spool-new-change` - Start a new change with artifact workflow
   - `spool-continue-change` - Continue working (create next artifact)

4. **How to use**
   - **Claude Code**: Skills are auto-detected, just ask Claude naturally
     - "I want to start a new Spool change to add X"
     - "Continue working on this change"
   - **Cursor**: Enable in Settings → Rules → Import Settings
...
1. **Batch mode refactor** - Making legacy `/spool:proposal` use schemas
...
1. `spool-new-change`, `spool-continue-change`, and `spool-apply-change` skills work end-to-end
2. `spool artifact-experimental-setup` creates skills in `.claude/skills/`
...
2. **Namespace in CLI** - Should experimental CLI commands be namespaced?
   - Current: `spool status`, `spool instructions` (no namespace)
   - Alternative: `spool opsx status` (explicit experimental namespace)
...
2. **Structured instructions output** ⭐ HIGH PRIORITY - KEEP
   - `spool instructions <artifact>` gave templates, output paths, and context in one call
...
3. **Simple scaffolding** ✅ WORKS WELL
   - `spool new change "name"` just worked - created directory structure without fuss
...
3. **Repetitive --change flag** ⚠️ MEDIUM PRIORITY
   ...
   - **Recommendation**: `spool use "tic-tac-toe-game"` to set context, then subsequent commands assume that change
...
4. **No validation feedback** 🔥 HIGH PRIORITY - ADD
   ...
   - **Recommendation**: Add `spool validate --change "name"` to check content quality
...
5. **Query-heavy, action-light CLI** 🔥 HIGH PRIORITY - ENHANCE
   ...
   - **Recommendation**: `spool create proposal --change "name"` could scaffold the file with template pre-filled, then user just edits
...
2. **No connection to implementation** 🔥 HIGH PRIORITY - ROADMAP ITEM
   ...
   - No `spool apply` or guidance on how to execute the tasks
   - User asked "would you like me to implement?" but that's outside Spool's scope currently
   - **Recommendation**: Add implementation bridge - either:
     - `spool apply` command to start execution phase
     - Clear handoff to existing `/spool:apply` workflow
...
**Problem:** CLI is mostly query-oriented. Agents run `spool status`, `spool next`, `spool instructions` but then must manually write files.

#### Decision 2a: Remove `spool next` command (RESOLVED)
...
**Current behavior:**
```bash
spool status --change "X"  # Shows: proposal (done), specs (ready), design (blocked), tasks (blocked)
spool next --change "X"    # Shows: ["specs"]  ← redundant
```
...
**Options considered:**
- Add `spool create <artifact>` commands that scaffold files with templates
...
|--------|--------------|-------|-------------|
| `new` | Create a change (scaffold directory) | `opsx:new` | `spool new change` |
| `continue` | Create next artifact (dependency-aware) | `opsx:continue` | `spool instructions` |
...
| `validate` | Check artifacts are correct/complete | TBD | `spool validate` |
| `archive` | Finalize and move to archive | existing | `spool archive` |
...
### Design: `spool-apply-change` Skill

#### Overview

The apply skill guides agents through implementing tasks from a completed (or in-progress) change. Unlike the old `/spool:apply` command, this skill:
...
1. **If no change name provided, prompt for selection**
   ...
   Run `spool list --json` to get available changes. Use **AskUserQuestion** to let user select.
...
2. **Get apply instructions**
   ...
   **Handle states:**
   - If blocked (missing artifacts): show message, suggest `spool-continue-change`
...
→ spool-apply-change: implements tasks 1, 2, 3, 4...
...
→ spool-apply-change: implements tasks 5, 6, 7
...
```bash
spool list --json                        # List changes for selection
spool status --change "<name>"           # Check artifact completion
spool instructions apply --change "<name>" # Get apply instructions (NEW)
# File reads via Read tool for proposal, specs, design, tasks
# File edits via Edit tool for checking off tasks
```

#### New CLI Command: `spool instructions apply`
...
### Context Files
- proposal: spool/changes/add-user-auth/proposal.md
- specs: spool/changes/add-user-auth/specs/**/*.md
- design: spool/changes/add-user-auth/design.md
- tasks: spool/changes/add-user-auth/tasks.md
...
- **Consistency** - same pattern as `spool instructions <artifact>`
...
#### Differences from Old `/spool:apply`

| Aspect | Old `/spool:apply` | New `spool-apply-change` |
|--------|----------------------|----------------------------|
...
1. **Add CLI command**: Add `spool instructions apply` to artifact-workflow.ts
...
5. ~~Remove `spool next` command (Decision 2a)~~ (Done)
6. ~~Add `spool instructions apply` CLI command~~ (Done)
7. ~~Create `spool-apply-change` skill~~ (Done)
