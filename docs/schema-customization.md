...
| All (if set) | `$XDG_DATA_HOME/spool/schemas/` |

### 2. Create the directory structure

```bash
# macOS/Linux example
mkdir -p ~/.local/share/spool/schemas/spec-driven/templates
```
...
# or
which spool && readlink -f $(which spool)

# Copy files from the package's schemas/ directory
cp <package-path>/schemas/spec-driven/schema.yaml ~/.local/share/spool/schemas/spec-driven/
cp <package-path>/schemas/spec-driven/templates/*.md ~/.local/share/spool/schemas/spec-driven/templates/
```
...
| **No diffing** | When upgrading spool, users can't see what changed in built-in templates |
...
### Option A: Single `spool schema` command
...
# Output: /Users/me/.local/share/spool/schemas/spec-driven/ (user override)
# Output: /usr/local/lib/node_modules/spool/schemas/spec-driven/ (built-in)

# Copy a built-in schema to user directory for customization
spool schema copy spec-driven
# Creates ~/.local/share/spool/schemas/spec-driven/ with all files
...
# Validate a schema
spool schema validate spec-driven
```

### Option B: Dedicated `spool customize` command
