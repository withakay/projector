...
|------|------------|---------|
| **Change** | A unit of work being planned (feature, refactor, migration) | `spool/changes/add-auth/` |
...
```
1. ${XDG_DATA_HOME}/spool/schemas/<name>/schema.yaml   # Global user override
...
```
1. ${XDG_DATA_HOME}/spool/schemas/<schema>/templates/<artifact>.md  # User override
...
| Responsibility | Approach |
|----------------|----------|
| Create changes | Create dirs under `spool/changes/<name>/` with README |
| Name validation | Enforce kebab-case naming |

**Key Paths:**

```
spool/changes/<name>/   → Change instances with artifacts (project-level)
```
...
| `list` | List all changes | EXISTS (`spool change list`) |
| `new <name>` | Create change | **NEW** (uses `createChange()`) |
| `init` | Initialize structure | EXISTS (`spool init`) |
...
│  │  ~/.local/share/ │   │  spool/changes/<name>/      │  │
│  │  spool/       │   │  - proposal.md, design.md      │  │
...
```
${XDG_DATA_HOME}/spool/schemas/<name>/schema.yaml   # User override
    ↓ (not found)
...
```
${XDG_DATA_HOME}/spool/schemas/<schema>/templates/<artifact>.md  # User override
    ↓ (not found)
...
~/.local/share/spool/           # Unix/macOS ($XDG_DATA_HOME/spool/)
%LOCALAPPDATA%/spool/           # Windows
...
# Or user override: ~/.local/share/spool/schemas/spec-driven/schema.yaml
name: spec-driven
version: 1
