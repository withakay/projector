...
- Refer to \`projector/AGENTS.md\` (located inside the \`projector/\` directory—run \`ls projector\` or \`projector update\` if you don't see it) if you need additional Projector conventions or clarifications.`;

const proposalGuardrails = `${baseGuardrails}\n- Identify any vague or ambiguous details and ask the necessary follow-up questions before editing files.
- Do not write any code during the proposal stage. Only create design documents (proposal.md, tasks.md, design.md, and spec deltas). Implementation happens in the apply stage after approval.`;

const proposalSteps = `**Steps**
1. Review \`projector/project.md\`, run \`projector list\` and \`projector list --specs\`, and inspect related code or docs (e.g., via \`rg\`/\`ls\`) to ground the proposal in current behaviour; note any gaps that require clarification.
2. Choose a unique verb-led \`change-id\` and scaffold \`proposal.md\`, \`tasks.md\`, and \`design.md\` (when needed) under \`projector/changes/<id>/\`.
...
7. Validate with \`projector validate <id> --strict\` and resolve every issue before sharing the proposal.`;


const proposalReferences = `**Reference**
- Use \`projector show <id> --json --deltas-only\` or \`projector show <spec> --type spec\` to inspect details when validation fails.
- Search existing requirements with \`rg -n "Requirement:|Scenario:" projector/specs\` before writing new ones.
...
5. Reference \`projector list\` or \`projector show <item>\` when additional context is required.`;

const applyReferences = `**Reference**
- Use \`projector show <id> --json --deltas-only\` if you need additional context from the proposal while implementing.`;

const archiveSteps = `**Steps**
1. Determine the change ID to archive:
   - If this prompt already includes a specific change ID (for example inside a \`<ChangeId>\` block populated by slash-command arguments), use that value after trimming whitespace.
   - If the conversation references a change loosely (for example by title or summary), run \`projector list\` to surface likely IDs, share the relevant candidates, and confirm which one the user intends.
   - Otherwise, review the conversation, run \`projector list\`, and ask the user which change to archive; wait for a confirmed change ID before proceeding.
   - If you still cannot identify a single change ID, stop and tell the user you cannot archive anything yet.
2. Validate the change ID by running \`projector list\` (or \`projector show <id>\`) and stop if the change is missing, already archived, or otherwise not ready to archive.
3. Run \`projector archive <id> --yes\` so the CLI moves the change and applies spec updates without prompts (use \`--skip-specs\` only for tooling-only work).
4. Review the command output to confirm the target specs were updated and the change landed in \`changes/archive/\`.
5. Validate with \`projector validate --strict\` and inspect with \`projector show <id>\` if anything looks off.`;

const archiveReferences = `**Reference**
- Use \`projector list\` to confirm change IDs before archiving.
- Inspect refreshed specs with \`projector list --specs\` and address any validation issues before handing off.`;

// Research command bodies - load from projector/commands/ or use inline
...
- Write findings to the appropriate file in projector/research/investigations/.`;
...
5. Write findings to \`projector/research/investigations/stack-analysis.md\`
...
5. Write findings to \`projector/research/investigations/feature-landscape.md\`
...
5. Write findings to \`projector/research/investigations/architecture.md\`
...
5. Write findings to \`projector/research/investigations/pitfalls.md\`
...
4. Write findings to \`projector/changes/<id>/reviews/security.md\`
...
5. Write findings to \`projector/changes/<id>/reviews/scale.md\`
...
5. Write findings to \`projector/changes/<id>/reviews/edge-cases.md\`
